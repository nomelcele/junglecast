(function(e){
	e.skdslider=function(t,n){
		...
		if(n){e.extend(r,n)}
		var i="ontouchstart"in window||window.DocumentTouch&&document instanceof DocumentTouch;e(t).wrap('<div class="skdslider"></div>');
		...
		if(r.animationType=="sliding"){
			o.each(function(){e(this).css({"float":"left",display:"block",position:"relative"})});
			var a=s.outerWidth()*o.size();
			s.find("ul.slides").css({position:"absolute",left:"0",width:a, height:"100%"});
			o.css({width:s.outerWidth(),height:"100%"});
			e(window).resize(function(){
				var e=s.outerWidth()*o.size();
				s.find("ul.slides").css({position:"absolute",left:"0",width:e, height:"100%"});
				o.css({width:s.outerWidth(),height:"100%"})
			})
		}
		...
		if(r.autoSlide==true){
			r.currentState="play";
			r.interval=setTimeout(function(){e.skdslider.playSlide(s,o,r)},r.delay)
		}
		if(r.pauseOnHover==true){
			o.hover(function(){
				if(r.autoSlide==true){r.currentState="pause";clearTimeout(r.interval)}
			},function(){
				if(r.autoSlide==true){
					r.currentState="play";
		...
		for(i=0;i<n.length;i++){
			...
			e.skdslider.playSlide=function(t,n,r,i){
				if(r.currentState=="play"&&r.running==false){
					...
					r.interval=setTimeout(function(){e.skdslider.playSlide(t,n,r)},r.delay)}};
